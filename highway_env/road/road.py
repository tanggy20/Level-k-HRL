...
class RoadNetwork:
    ...
    def bfs_paths(self, start: str, goal: str) :
        """
...
class Road:
    ...
    def level2_act(self, state_list) -> None:
        other_vehicle_list = self.vehicles[1:]
        index = 0
        for vehicle in other_vehicle_list:
            ...
            if type(state) == type((1,)):
                state = state[0]
            action = vehicle.predict(state)
            vehicle.act(action)
    
    def level1_act(self, state_list) -> None:
        model = DQN.load("dqn_checkpoints/level1_test_1.0/level1_final_model_2_999", device="cuda:0", weights_only=False)
        # model = DQN.load("dqn_checkpoints/level1_with_dl/level1_model", device="cuda:0", weights_only=False)
        other_vehicle_list = self.vehicles[1:]
        index = 0
        for vehicle in other_vehicle_list:
            state = state_list[index]
            ...
            # state_tensor = torch.tensor(state, dtype=torch.float32).unsqueeze(0).to("cuda:0")
            # print(model.q_net(state_tensor))
            vehicle.act(action)

    def level0_act(self, act_list) -> None:
        other_vehicle_list = self.vehicles[1:]
        index = 0
        for vehicle in other_vehicle_list:
            action = act_list[index]
            index += 1
            vehicle.act(action)
