...
class AbstractEnv(gym.Env):
    ...
    PERCEPTION_DISTANCE = 3.0 * Vehicle.MAX_SPEED
    ...
    def define_spaces(self) -> None:
        ...
        self.observation_type1 = observation_factory(self, self.config["observation1"])
    ...
    def step(self, action: Action) -> tuple[Observation, float, bool, bool, dict]:
        ...
        if self.steps % 1000 == 0:
            print("step: {step}, reward: {reward}".format(self.steps, reward))
        if self.render_mode == "human":
            self.render()
    ...
    def _simulate(self, action: Action | None = None) -> None:
        ...
        for frame in range(frames):
            ...
            ###环境决策
            if self.config["training_level"] == 2:
                other_vehicle_list = self.road.vehicles[1:]
                state_list = []
                for vehicle in other_vehicle_list:
                    self.observation_type1.observer_vehicle = vehicle
                    obs = self.observation_type1.observe()
                    self.observation_type1.observer_vehicle = None
                    state_list.append(obs)
                self.road.level1_act(state_list)  
            elif self.config["training_level"] == 3:
                other_vehicle_list = self.road.vehicles[1:]
                state_list = []
                for vehicle in other_vehicle_list:
                    self.observation_type.observer_vehicle = vehicle
                    obs = self.observation_type.observe()
                    self.observation_type.observer_vehicle = None
                    state_list.append(obs)
                self.road.level2_act(state_list)
            elif self.config["training_level"] == 1:
                # other_vehicle_list = self.road.vehicles[1:]
                # state_list = []
                # for vehicle in other_vehicle_list:
                #     self.observation_type.observer_vehicle = vehicle
                ...
                #     self.observation_type.observer_vehicle = None
                #     state_list.append(obs)
                # self.road.level1_act(state_list)  ### 设置环境车辆为一级决策验证交互性 2025.10.24
                ...
                #         if (v.lane_index == vehicle.lane_index) and ((v.position[0] - vehicle.position[0]) > 0) \
                #                 and ((v.position[0] - vehicle.position[0]) < 30) and (v.speed < vehicle.speed):
                #             flag = 1
                #     if flag:
                #         state_list.append(4)
                #     else:
                #         state_list.append(1)
                # self.road.level0_act(state_list)
                self.road.act()
            else:
                self.road.act()
