...
### Hierarchical Action Factory ### // 2025-07-09

class HierarchicalMetaAction(ActionType):
    """
    An action type that allows for hierarchical meta-actions, where each agent can have its own action type.
    """

    GOALS = {0: 'LANE_LEFT', 1: 'IDLE', 2: 'LANE_RIGHT'}
    PRIMS = {0: 'SLOWER', 1: 'IDLE', 2: 'FASTER'}
    PAIRS = [(g, u) for g in range(len(GOALS)) for u in range(3)]

    def __init__(self, env: AbstractEnv, **kwargs) -> None:
        """
        Create a hierarchical meta-action space.

        :param env: the environment
        :param action_config: configuration for the action types of each agent
        """
        super().__init__(env)
        self.pairs = [(g, u) for g in self.GOALS for u in self.PRIMS]
    
    def space(self) -> spaces.Space:
        """
        Define the action space as a tuple of discrete spaces for goals and primitives.
        """
        return spaces.Discrete(len(self.pairs))
    
    @property
    def vehicle_class(self) -> Callable:
        """
        Return the vehicle class for the hierarchical meta-action.
        """
        return MDPVehicle  # Assuming MDPVehicle is suitable for hierarchical actions
    
    def act(self, action: int | np.ndarray) -> None:
        """
        Execute the hierarchical meta-action based on the action index.

        :param action: the action index corresponding to a goal-primitive pair
        """
        raise NotImplementedError("HierarchicalMetaAction.act method is not implemented yet.")
    
    def get_available_actions(self) -> list[int]:
        """
        Get the list of currently available hierarchical meta-actions.

        This can be extended to check conditions for each goal-primitive pair.
        """
...
def action_factory(env: AbstractEnv, config: dict) -> ActionType:
    ...
    elif config["type"] == "HierarchicalMetaAction":
        return HierarchicalMetaAction(env, **config)
    else:
