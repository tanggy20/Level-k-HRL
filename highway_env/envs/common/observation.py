...
class KinematicObservation(ObservationType):
    ...
    def observe(self) -> np.ndarray:
        ...
        ego_lane_index = self.observer_vehicle.lane_index[2]
        left_front = None
        left_rear = None
        front = None
        rear = None
        ...
        if close_vehicles:
            for vehicle in close_vehicles:
                vehicle_lane_index = vehicle.lane_index[2]

                if vehicle_lane_index == ego_lane_index - 1:
                    if vehicle.position[0] > self.observer_vehicle.position[0]:
                        if left_front is None:
                            left_front = vehicle
                    else:
                        if left_rear is None:
                            left_rear = vehicle
                elif vehicle_lane_index == ego_lane_index:
                    if vehicle.position[0] > self.observer_vehicle.position[0]:
                        if front is None:
                            front = vehicle
                    else:
                        if rear is None:
                            rear = vehicle
                elif vehicle_lane_index == ego_lane_index + 1:
                    if vehicle.position[0] > self.observer_vehicle.position[0]:
                        if right_front is None:
                            right_front = vehicle
                    else:
                        if right_rear is None:
                            right_rear = vehicle
                if all([left_front, left_rear, front, rear, right_front, right_rear]):
                    break

        observed_vehicles = [left_front, left_rear, front, rear, right_front, right_rear]
        ...
        for vehicle in observed_vehicles:
            if vehicle is None:
                ...
                origin = self.observer_vehicle if not self.absolute else None
                vehicles_df = pd.DataFrame.from_records(
                    [vehicle.to_dict(origin, observe_intentions=self.observe_intentions)]
                )[self.features]
                df = pd.concat([df, vehicles_df], ignore_index=True)
        ...
        obs = df.values.flatten()
        return obs.astype(self.space().dtype)


### 加上最大限速 2025-10-31

class KinematicSpeedObservation(ObservationType):
    """Observe the kinematics of nearby vehicles."""
    ...
    def space(self) -> spaces.Space:
        return spaces.Box(
            shape=(self.vehicles_count * len(self.features)+1,),
            low=-np.inf,
            high=np.inf,
            dtype=np.float32,
        )
    ...
    def normalize_obs(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        return df
    def observe(self) -> np.ndarray:
        ego_max_target_speed = self.observer_vehicle.max_speed_limit
        normalized_target_speed = ego_max_target_speed / Vehicle.MAX_SPEED
        g = np.array([normalized_target_speed], dtype=np.float32)
        if not self.env.road:
            zero = np.zeros(self.vehicles_count * len(self.features), dtype=np.float32)
            return np.concatenate([zero, g], axis=0).astype(np.float32)
        ...
        # Flatten the observation
        obs = df.values.flatten()
        obs = np.concatenate([obs, g], axis=0)
        return obs.astype(self.space().dtype)
...
def observation_factory(env: AbstractEnv, config: dict) -> ObservationType:
    ...
    elif config["type"] == "KinematicSpeedObservation":
        return KinematicSpeedObservation(env, **config)
    else:
