...
if __name__ == "__main__":
    ...
    # ---------- 4. 画图：跟驰速度 v（HighD vs Sim） ----------
    out_v_cf = os.path.join(SAVE_DIR, "cf_speed_sim_vs_highd.png")
    plot_hist_density_compare(
        data_sim=v_cf_sim,
        data_highd=cf_speed_highd,
        filename=out_v_cf,
        xmin=22,
        xmax=SPEED_XMAX,
        ymax=YMAX_SPEED,
    ...
    print(f"已生成跟驰速度对比图: {out_v_cf}")


    # ---------- 6. 画图：换道速度 v（HighD vs Sim） ----------
    out_v_lc = os.path.join(SAVE_DIR, "lc_speed_sim_vs_highd.png")
    plot_hist_density_compare(
        data_sim=v_lc_sim,
        data_highd=lc_speed_highd,
        filename=out_v_lc,
        xmin=22,
        xmax=SPEED_XMAX,
        ymax=YMAX_SPEED,
