...
env = gym.make("level2-v0", render_mode="human")

# 确保启用轨迹显示
env.unwrapped.config["show_trajectories"] = False   
env.unwrapped.config["render_agent"] = True
...
try:
    model = DQN.load(
        "dqn_checkpoints/level2_efal/level2_efal_with_bc.zip",
        custom_objects={
    ...
    # Fallback: load without environment and set it manually
    model = DQN.load("dqn_checkpoints/level2_efal/level2_efal_500000_steps.zip")
    model.set_env(env)
for episode in trange(1800, desc="Episode"):
    ...
    while not (done or truncated):
        ...
        if front_vehicle and front_vehicle.position[0] - env.unwrapped.vehicle.position[0] < 50:
            ...
            if front_vehicle:
                dhw_list.append(front_vehicle.position[0] - env.unwrapped.vehicle.position[0])
            # acc
