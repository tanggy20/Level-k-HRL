...
env = gym.make("test2-v0", render_mode="human")
...
for episode in trange(1800, desc="Episode"):
    ...
    while not (done or truncated):
        ...
        front_vehicle, rear_vehicle = env.unwrapped.vehicle.road.neighbour_vehicles(
            env.unwrapped.vehicle, env.unwrapped.vehicle.lane_index
        )
        if front_vehicle and front_vehicle.position[0] - env.unwrapped.vehicle.position[0] < 50:
            steps += 1
            # 遍历所有车辆
            interaction = 0
            for vehicle in env.unwrapped.road.vehicles:
                ...
                if abs(vehicle.position[0] - env.unwrapped.vehicle.position[0]) < 30 and abs(vehicle.lane_index[2] - env.unwrapped.vehicle.lane_index[2]) <= 1:
                    interaction += 1
            interact_list.append(interaction)
            # ttc
            front_vehicle, rear_vehicle = env.unwrapped.road.neighbour_vehicles(
                env.unwrapped.vehicle, env.unwrapped.vehicle.lane_index
            )
            if front_vehicle and (env.unwrapped.vehicle.speed - front_vehicle.speed) > 0:
                ttc = (front_vehicle.position[0] - env.unwrapped.vehicle.position[0]) / (env.unwrapped.vehicle.speed - front_vehicle.speed)
                if ttc > 100:
                    ttc = 100
                ttc_list.append(ttc)
            # acc
            acc_list.append(env.unwrapped.vehicle.action['acceleration'])
            # steering
            steer_list.append(env.unwrapped.vehicle.action['steering'])
            # speed
            speed_list.append(env.unwrapped.vehicle.speed)
env.close()
